...
public class Team {
    ...
    String [] playersIndia={"i1","i2","i3","i4","i5","i6","i7","i8","i9","i10","i11"};
    String [] playersPakistan={"p1","p2","p3","p4","p5","p6","p7","p8","p9","p10","p11"};
    ...
    public ArrayList<Player> getDetails(){
        for(int i=0;i<11;i++){
            if(team_name.equals("india")){
                    Player p = new Player(playersIndia[i]+i, playersIndia[i]);
                    team.add(p);
            }
            else if(team_name.equals("pakistan")){
                Player p = new Player(playersPakistan[i]+i, playersPakistan[i]);
                team.add(p);
            }
        }
        return team;
